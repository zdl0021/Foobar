...
# Thumb wrestle match between 2 guards
# Evaluated in constant time
def thumbWrestle(x, y):
    # simplify ratio x / y, take sum of components
    ratioSum = (x + y) // gcd(x, y)
    
    # if power of 2, guards eventually end match
    if ratioSum & (ratioSum - 1) == 0:
        return 0
    else:
        return 1
...
def minGuards(pairs):
    ...
    for i in range(1, len(pairs[0])):
        ...
        while pos != len(pairs):
            ...
            if pos == len(pairs):
                break
            for j in range(1, len(pairs[pos])):
                if pairs[pos][j] > 0:
                    for pair2 in pairs[pos + 1:]:
                        if pair2[0] > 0:
                            if pair2[0] == pairs[pos][j]:
                                pair2[0] *= -1
                                break
                            for k in range(1, len(pair2)):
                                if pair2[k] == pairs[pos][j]:
                                    pair2[k] *= -1
                                    break
                    pairs[pos][j] *= -1
                    tempSmallest -= 2
...
# Approach: review all possible matchups,
# all ((n-1)*n)/2 of them
def answer(banana_list):
	...
	for i in range(0, len(banana_list)):
		pairs.append([banana_list[i]])
		for j in range(i + 1, len(banana_list)):
			if thumbWrestle(banana_list[i], banana_list[j]):
				pairs[pos].append(banana_list[j])
		pos += 1
	return minGuards(pairs)
